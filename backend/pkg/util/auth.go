...
func (v *FirebaseVerifier) VerifyIDToken(idToken string) (*AuthResult, error) {
	...
	if err != nil {
		return nil, goerr.Wrap(err, "Failed to get auth client")
	}
	...
	if !ok {
		return nil, goerr.New("Failed to assert type [firebase]: %v", claims)
	}

	provider, ok := firebaseField["sign_in_provider"].(string)
	if !ok {
		return nil, goerr.New("Failed to assert type: %v", firebaseField["sign_in_provider"])
	}

	identities, ok := firebaseField["identities"].(map[string]interface{})
	if !ok {
		return nil, goerr.New("Failed to assert type: %v", firebaseField["identities"])
	}
...
func extractAuthUID(identities map[string]interface{}, provider string) (string, error) {
	...
	if !ok {
		return "", goerr.New("Type assertion error: %v", identities[provider])
	}

	uid, ok := arr[0].(string)
	if !ok {
		return "", goerr.New("Type assertion error: %v", arr[0])
	}
...
func extractAuthProvider(provider string) (string, error) {
	...
	case "twitter.com":
		return "twitter", nil
	default:
		return "", goerr.New("Unknown provider: %s", provider)
	}
}
